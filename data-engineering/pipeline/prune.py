...
window they belong to is built. Keeping thirty days of them is enough to rebuild
recent history and to show the lake layer is real.
...
def main(argv: list[str] | None = None) -> int:
    ...
    with psycopg.connect(dsn) as connection:
        ...
        # VACUUM cannot run inside a transaction block, and without it the
        # deleted rows are dead tuples that still occupy the disk the delete was
        # meant to free — which would leave the database exactly as full.
        connection.commit()
        with connection.cursor() as cursor:
            connection.autocommit = True
            for table, _ in EVENT_TABLES:
                cursor.execute("SELECT to_regclass(%s)", (f"public.{table}",))
