 ...
 */
/**
 * What in this city is generated — which is no longer everything.
 *
 * It said DEMO DATA across the whole city, driven by `city.demoData`. That was
 * true when every feed was the generator's. It stopped being true when air
 * quality became real, and it now hides the platform's strongest claim under a
 * label that reads as "none of this counts".
 *
 * So it names the mix rather than the verdict. Which feeds are on which side is
 * not written here: this badge carried its own copy of that sentence, and when
 * traffic stopped being generated it went on saying it was — the same drift that
 * put a stale disclosure in five files and is the reason DATA_DISCLOSURE exists
 * at all. The hover text is that constant, so there is one place to change and
 * this cannot fall behind it again.
 *
 * The wording lives there; every figure carries its own provenance where it is
 * displayed. This is the summary, not the disclosure.
 */
